...
Mint can be installed using [Homebrew](https://brew.sh/) with `brew install mint`, among [other methods](https://github.com/yonaskolb/Mint#installing). Bundler can be installed with `gem install bundler`. To install the Ruby tools, run `bundler install`.

The Xcode project file is not included in this repository, and is generated by running [XcodeGen](https://github.com/yonaskolb/XcodeGen): `mint run yonaskolb/XcodeGen`.

To install the various iOS libraries used by Reading List, run: `pod install`.
